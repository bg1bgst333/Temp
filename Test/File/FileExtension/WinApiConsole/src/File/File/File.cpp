...
int _tmain() {
	...
	TCHAR tszExtension[128] = { 0 };	// 拡張子tszExtensionを0で初期化.
	...
	// ファイル拡張子の取得.
	_tcscpy_s(tszExtension, PathFindExtension(tszFileName));	// PathFindExtensionで拡張子を取得し, _tcscpy_sでtszExtensionにコピー.
	_tprintf(_T("tszExtension = %s\n"), tszExtension);	// _tprintfでtszExtensionを出力.

...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nShowCmd) {
	...
	wc.lpszClassName = _T("Window");	// ウィンドウクラス名"Window"
	...
	// ウィンドウの作成
	hWnd = CreateWindow(wc.lpszClassName, _T("Window"), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, hInstance, NULL);	// CreateWindowでウィンドウクラス"Window"なウィンドウ"Window"を作成.
	if (hWnd == NULL){	// hWndがNULLならウィンドウ作成失敗.

		// ウィンドウ作成失敗エラー処理
		MessageBox(NULL, _T("CreateWindow failed!"), _T("Window"), MB_OK | MB_ICONHAND);	// MessageBoxで"CreateWindow failed!"を表示.
		return -2;	// -2を返して異常終了.
